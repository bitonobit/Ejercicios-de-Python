...
de código
'''
'''Haciendo uso de barra inclinada se puede romper el código en 
varias líneas, lo que en determinados casos hace que 
el código sea mucho más legible.'''
x = 1 + 2 + 3 + 4 +\
 5 + 6 + 7 + 8
#Se puede omitir el \ si colocas ()
x = (1 + 2 + 3 + 4 +
 5 + 6 + 7 + 8)
#Otro ejemplo
def funcion(a, b, c):
...
'''
Dudas:
Python permite cambiar de tipo despues de haber creado la variable, eso no lo hace debilmente tipado?
'''
...
# pondremos en nuestro código que el 
...
# news/metodos-de-string-de-python-explicados-con-ejemplo/
